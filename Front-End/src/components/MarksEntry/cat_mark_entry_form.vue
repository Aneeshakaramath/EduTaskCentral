...
async function addCatMarks() {
    ...
    const response = await questionPaperStore.addCatMarks(JSON.parse(JSON.stringify(addMarksPayload)));
    if(response.courseId == addMarksPayload.courseId || response.courseId!=null) {
        swal("Success!", "cat marks has been added successfully", "success");
